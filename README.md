# VideoMerge

瀏覽器端影片工具：預覽每段影片的**首幀 / 尾幀**，並可將**多段影片合併**成一個 MP4；也支援**圖片 + MP3 → 靜態影片**（直式／橫式依圖片）。

所有處理都在本機完成（Canvas 擷取影格 + FFmpeg.wasm），不會上傳到伺服器。

## 功能

- 拖曳或選擇多個**影片**或**圖片**（JPG / PNG / WebP 等）
- 自動顯示每段的首幀、尾幀縮圖與時長 / 解析度（圖片顯示靜態預覽與直式／橫式）
- **圖片 + MP3 成片**：靜態畫面長度預設對齊音軌；**輸出保留原圖像素與長寬比**（不強制壓到 1080p；長邊超過 4096 才等比縮小，避免模糊）
  - 例：4032×3024 原樣輸出、6000×4000 → 4096×2730、800×600 不放大
- 調整合併順序（上移 / 下移）
- 可勾選「不要聲音」輸出無音軌影片
- **自訂音軌**：可用 MP3 等音訊取代原影片聲音（短則循環、長則裁切）；純圖片時作為成片長度依據
- **語音稿字幕**：貼上／上傳講稿，依影片時長自動切句上字幕（免下載模型；亦支援 SRT/VTT）
- **依音軌自動辨識字幕**：Whisper 辨識 MP3（較慢；模型首次下載後會快取）
- **延長 / 循環**：依「重複次數」或「目標時長」自動循環整段並裁切
- 一鍵合併為 MP4 並預覽、下載

## 桌面版（Avalonia）

同功能的 Windows 桌面程式在 `VideoMerge.Avalonia/`：加入影片／圖片、預覽首尾幀、**單獨播放每段影片**、自訂音軌、語音稿字幕、循環延長，並以本機 FFmpeg 合併為 MP4。

**需求：** [.NET 8 SDK](https://dotnet.microsoft.com/download) 與 [FFmpeg](https://ffmpeg.org/)（`ffmpeg` / `ffprobe` 在 PATH，或啟動後按「指定 FFmpeg」）。

```bat
start-desktop.bat
```

或：

```bash
dotnet run --project VideoMerge.Avalonia -c Release
```

## 本機啟動

**需求：** [Node.js](https://nodejs.org/) 18+（含 npm）

### 方式一：一鍵腳本（建議）

| 環境 | 指令 |
|------|------|
| **Linux / macOS / Git Bash** | `./start.sh` |
| **Windows CMD / 雙擊** | `start.bat`（請用雙擊，或在專案目錄執行 `start.bat`） |

腳本會自動：
1. 檢查 Node / npm  
2. 若尚未安裝依賴則執行 `npm install`  
3. 啟動開發伺服器並嘗試開啟瀏覽器（預設 `http://localhost:5173`）

自訂埠號：

```bash
PORT=3000 ./start.sh          # bash
set PORT=3000 && start.bat    # Windows cmd
```

### 方式二：npm

```bash
npm install
npm start          # 等同開發伺服器 + 自動開瀏覽器
# 或
npm run dev
```

### 建置預覽

```bash
npm run build
npm run preview
```

## 技術

| 項目 | 說明 |
|------|------|
| Vite | 開發與建置 |
| HTML5 Video + Canvas | 擷取首尾幀 |
| FFmpeg.wasm | 標準化解析度 / 幀率後串接影片 |

合併時會將各片段標準化為同一畫幅、30fps、H.264 + AAC：畫幅依第一段素材**原生解析度**（長邊上限 4096，從不無故放大），縮放用 lanczos；靜態圖另用 `-tune stillimage` + 較低 CRF，減少模糊與色塊。

## 注意事項

- 首次合併需下載 FFmpeg 核心（約數十 MB），之後可走快取
- 長影片或很多片段時，瀏覽器內轉檔會較慢，屬正常現象
- 建議使用較新的 Chrome / Edge / Firefox
